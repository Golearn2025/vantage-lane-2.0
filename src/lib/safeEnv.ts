...
const buildFallback = 'mock://build-time-fallback'
...
export const safeEnv = envsafe({
  ...
  NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY: isBuild
    ...
    : str(),
  STRIPE_SECRET_KEY: isBuild
    ? str({ devDefault: 'sk_test_mock_key' })
    : str(),
  STRIPE_WEBHOOK_SECRET: isBuild
    ? str({ devDefault: 'whsec_mock_secret' })
    : str(),

  // Email (Critical)
  RESEND_API_KEY: isBuild
    ? str({ devDefault: 're_mock_api_key' })
    : str(),

  // Google Maps (Critical)
  GOOGLE_MAPS_API_KEY: isBuild
    ? str({ devDefault: 'AIzaSyMockKey' })
    : str(),
